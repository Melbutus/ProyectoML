...
# uvicorn API.main:app --reload para ejecutar la api
...
# 3. Score por título
df_score = pd.read_parquet(r'score_titulo.parquet')
...
def score_titulo(titulo: str):    
    ...
    if not film.empty:
        # Manejar valores nulos para la fecha de estreno y el score
        ...
        return {"mensaje": f"La película '{titulo}' fue estrenada en el año {year} con un score de {score}"}
    else:
...
df_actores = pd.read_parquet('actor_dataset.parquet')
...
df_directores = pd.read_parquet('director_dataset.parquet')
...
def get_director(nombre_director: str):
    try:
        ...
        for _, film in director_films.iterrows():
            ...
            budget = film['budget'] if pd.notnull(film['budget']) and film['budget'] > 0 else "No disponible"
            revenue = film['revenue'] if pd.notnull(film['revenue']) and film['revenue'] > 0 else "No disponible"
            
            if budget != "No disponible" and revenue != "No disponible":
                retorno = round(film['revenue'] / film['budget'], 2)
                total_return += retorno
                num_peliculas_con_retorno += 1
                peliculas_con_datos.append(f"- Título: {title}\n  Fecha de lanzamiento: {release_date}\n  Retorno: {retorno}\n  Costo: {budget}\n  Ganancia: {revenue}")
            else:
                # Guardar película con datos incompletos
                peliculas_sin_datos.append(f"- Título: {title}\n  Fecha de lanzamiento: {release_date}")
...
df_peliculas = pd.read_parquet('dataset_completo.parquet')
